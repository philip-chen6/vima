...
We introduce VIMA (\textbf{V}erifiable \textbf{I}nspection via \textbf{N}eural
...
\begin{enumerate}
  \item We formalize OSHA CFR~29~Part~1926 rules as verifiable binary
        reward functions and formalize how they could drive VLM-based
        spatial reasoning via GRPO-style reinforcement (training left to
        future work; this paper evaluates inference-time augmentation).
...
DeepSeek-R1~\cite{guo2025deepseekr1} demonstrated that restricting
RL training to tasks where reward can be computed deterministically---
mathematical proofs, code execution results, structured factual
claims---eliminates reward model gaming as a failure mode.
...
Liu et al.~\cite{liu2025grpo} formalize the GRPO convergence dynamics,
showing that mean-centered loss converges to a fixed point strictly
improving on the reference policy.
Our reward design borrows their rollout aggregation strategy for multi-frame
...
Unlike Ego4D, which targets activity recognition, VIMA targets
...
Safe-Construct~\cite{chharia2025safeconstruct} (CVPRW 2025) reframes
violation detection as a 3D multi-view engagement task, achieving 7.6\%
improvement over prior state-of-the-art by reasoning over spatial
worker-object context.

\textbf{VIMA differs from Safe-Construct in dataset, task scope, and
temporal modeling.}
Safe-Construct targets \emph{safety violation detection} on multi-view
RGB rigs with synchronized cameras; its output is a violation
classification useful for retrospective auditing.
VIMA targets \emph{temporal spatial reasoning} on egocentric monocular
video: understanding how worker--equipment--zone relationships evolve
across frames, with event-memory augmentation that supplies the VLM with
a structured changelog of scene state.
The two systems address different stages of construction intelligence:
Safe-Construct closes the retrospective audit loop; VIMA closes the
real-time situational awareness loop.
VIMA's event-memory layer yields a measured +32.0\% composite
improvement in spatial question answering over clip-only baselines in our
A/B evaluation (Experiment~\ref{sec:exp4}); this comparison is against a
VLM-only baseline, not against Safe-Construct.
...
\cref{fig:pipeline} shows the end-to-end VIMA pipeline.

\begin{figure}[t]
  \centering
  \fbox{\rule{0pt}{4cm}\rule{0.9\linewidth}{0pt}}
  \Description{Block diagram of the VIMA pipeline showing sensor fusion,
               VLM judge, and reward function components.}
  \caption{VIMA pipeline: egocentric fisheye frame and LiDAR point
...
A core claim of VIMA is that every reward signal maps to a specific,
citable regulation.
...
We compare VIMA's automatic CII tier assignment against a manual
...
\begin{table}[t]
  \centering
  \caption{VIMA CII classification vs.\ manual annotation (25 frames).
...
receives a temporal event log derived from the VIMA change-detection
...
\begin{table}[h]
  \centering
  \caption{A/B evaluation composite scores (0--1) by question.
           Composite = weighted mean of specificity, calibration, and
           temporal grounding per question.
           Temporal grounding is 0.0 for VLM-only by construction.
           Scorer: Claude Sonnet~4.6 rubric judge (same model, separate call).}
  \label{tab:ab_eval}
  \small
  \begin{tabular}{@{}lcc@{}}
    \toprule
    \textbf{Question} & \textbf{VLM-only} & \textbf{VLM+Memory} \\
    \midrule
    Q1: Worker position / posture & 0.733 & 0.840 \\
    Q2: Material layout           & 0.600 & 0.867 \\
    Q3: Structural change         & 0.467 & 0.720 \\
    Q4: Safety distances          & 0.667 & 0.933 \\
    Q5: Spatial reasoning         & 0.533 & 0.600 \\
    \midrule
    \textbf{Mean composite}       & \textbf{0.600} & \textbf{0.792} \\
    \textit{Improvement}          & \multicolumn{2}{c}{\textbf{+32.0\%}} \\
    \bottomrule
  \end{tabular}
\end{table}

\textbf{Interpretation.}
Memory augmentation yields a +32.0\% composite improvement (0.600
$\rightarrow$ 0.792), with all 5/5 questions improving under the
augmented condition.
The largest per-question lifts are on structural change detection (+54\%)
and safety distance estimation (+40\%), where temporal context from prior
frames provides the grounding the VLM needs to avoid confident but
unverifiable distance claims.
The temporal grounding dimension is 0.0 for VLM-only by construction:
without event memory, the model has no basis for temporal claims and
cannot be penalized for fabricating them.
Rubric scoring was performed by a separate Claude Sonnet~4.6 call
acting as judge; the judge prompt, five question--answer pairs, and
per-question scores are released in the project repository
(\url{https://github.com/philip-chen6/vinna}).
...
predicates, VIMA produces a reward signal that is consistent across
...
We have presented VIMA, a self-supervised spatial safety reasoning system
...
\bibitem{liu2025grpo}
T.~Liu, L.~Bai, and X.~Jiang.
\newblock {GRPO}: Group relative policy optimization for mathematical
  reasoning.
\newblock \emph{arXiv:2402.03300}, 2025.

\bibitem{chharia2025safeconstruct}
A.~Chharia, T.~Ren, T.~Furuhata, and K.~Shimada.
\newblock Safe-Construct: Redefining construction safety violation recognition
  as a 3D multi-view engagement task.
\newblock In \emph{Proc.\ CVPR Workshops}, 2025.
