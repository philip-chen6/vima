...
\textbf{VINNA differs from Safe-Construct in task, threat model, and
output type.}
Safe-Construct is a \emph{violation detection} system: it classifies
whether a safety code is violated, with no gaming incentive.
VINNA targets \emph{spatial reasoning with event memory}: understanding
worker--equipment--zone relationships over time, enabling progress
monitoring, distance estimation, and temporal change detection.
Where Safe-Construct produces a binary compliance flag, VINNA produces a
structured spatial scene graph that supports multiple downstream
applications without task-specific training.
Critically, VINNA's event-memory augmentation layer—not present in any
prior construction CV system—yields a measured +33.2\% composite
improvement in spatial question answering over clip-only baselines
(Experiment~\ref{sec:exp4}).
