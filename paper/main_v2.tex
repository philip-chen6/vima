...
\title{VINNA: Evidence-Anchored Reward Attribution for Construction Productivity Under Adversarial Gaming}
...
We propose VINNA, an evidence-anchored reward attribution pipeline that treats the productivity classifier not as a decision authority but as a \emph{witness} whose claims must pass through independently verifiable gates before any payout is issued.
...
We argue that spatial anchoring is a \emph{prerequisite}---not an accuracy improvement---for any construction reward system that must withstand adversarial gaming, and present VINNA as a design pattern and prototype whose central hypothesis remains to be tested in a controlled human study.
\end{abstract}
...
We present VINNA as a design pattern and prototype; the hypothesis that spatial anchoring improves human reviewer accuracy remains to be tested in a controlled study, for which we propose a protocol in Section~\ref{sec:exp2}.
...
Ego-Exo4D~\cite{grauman2024egoexo4d} extended this paradigm with simultaneous egocentric and exocentric capture of skilled tasks including carpentry, demonstrating that expert commentary and paired views reduce label ambiguity---the multi-witness philosophy that VINNA operationalizes for construction rewards.
...
\textbf{VINNA differs from Safe-Construct in task, threat model, camera constraint, and output type.}
...
VINNA targets \emph{productivity reward attribution}, where the adversarial threat model is inverted: a financially-motivated worker actively seeks to trigger false-positive productive labels.
Where Safe-Construct operates on multi-view synchronized camera rigs with trusted operators, VINNA runs on a single egocentric headcam worn by the incentivized actor---a monocular constraint that makes SfM structurally harder than multi-view with known baselines.
...
VINNA explicitly separates these into semantic anchors and activity proxies, blocking reward issuance until both streams concur under human review.
Kim et al.~\cite{kim2025optimizing} fine-tune a VLM with RAG for OSHA compliance monitoring; VINNA treats such an external VLM as an optional witness that augments rather than replaces local evidence.
...
Akhavian and Behzadan~\cite{akhavian2016smartphone} demonstrated smartphone-based construction activity recognition using sensor data and CII taxonomy labels, establishing the closest prior baseline for wearable-sensor CII classification that VINNA extends to video.
...
\textbf{VINNA extends verifiable rewards from symbolic domains (math, code) to physical labor.}
...
Egocentric multi-view reasoning~\cite{egocentricmultiview2025} shows that VLMs transferring spatial understanding across viewpoints still require explicit geometric grounding---VINNA addresses this by anchoring every candidate window to a COLMAP keyframe and camera pose, making spatial context replayable rather than implicit in model weights.
...
Ye et al.~\cite{ye2025scantobim} updated this with scan-to-BIM as the off-chain oracle, demonstrating that the bottleneck is not the payment rail but the trustworthiness of the progress signal---exactly the gap that VINNA's evidence ledger targets.
...
VINNA comprises five components arranged in a processing pipeline.
...
\textbf{Strategy B (VINNA three-gate, optimistic assumptions).}
...
\textbf{Strategy B$'$ (VINNA three-gate, conservative assumptions).}
...
B: VINNA (optimistic) & 0.0015 & 0.018 & 0.018 & 200$\times$ \\
B$'$: VINNA (conserv.)& 0.143  & 1.72  & 0.854 & 2.1$\times$ \\
\bottomrule
\end{tabular}
\label{tab:gaming}
\end{table}
...
\textbf{Measuring $p_{\mathrm{forge}}$ empirically, via a deception study with confederate workers, is the highest-priority next step for validating VINNA's threat model.}
...
Table~\ref{tab:comparison} positions VINNA against existing construction monitoring approaches.
...
\textbf{VINNA (this work)} & \textbf{Yes} & \textbf{Yes} & \textbf{Yes} & \textbf{Per-window} & \textbf{Yes} \\
\bottomrule
\end{tabular}
\label{tab:comparison}
\end{table}

VINNA is the only system that combines on-device execution, evidence-anchored reward attribution, and an explicit adversarial threat model.
This comparison is qualitative; VINNA's quantitative advantage over these systems has not been measured.
...
\end{itemize}
VINNA provides infrastructure for both, but only the first is fully automated.
...
VINNA demonstrates that the bottleneck for AI-driven construction productivity incentives is not classification accuracy---it is evidence architecture.
...
We present VINNA as a design pattern with a working prototype, not a fully validated system.
