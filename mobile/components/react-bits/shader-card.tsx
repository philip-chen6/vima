...
export interface ShaderCardProps {
  ...
  blur?: number;
  opacity?: number;
  /** when true, fills the parent (ignores width/height numerics) */
  fill?: boolean;
...
const ShaderCard: React.FC<ShaderCardProps> = ({
  ...
  // live-updatable refs so prop changes don't teardown the WebGL context
  const speedRef = useRef(speed);
  const uniformsRef = useRef<Record<string, { value: unknown }> | null>(null);

  // structural effect — initialize Three.js once. Re-runs only on truly structural changes.
  useEffect(() => {
    ...
    const defaultFragmentShader = `
      ...
      // dampened: was 1.25 (volatile) — calmer base motion
      #define NIGHTSPEEDBONUS 0.55
      #define SHAPE 0
      ...
      void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
        ...
        if(BREATHWILDNESS == 1) {
          ...
          // dampened: was sin(time*3.0) + pow(.,4) + 0.3-magnitude — much subtler now
          float whiteFlash =  sin(time*1.0);
          whiteFlash = pow(whiteFlash, 6.0);
          effectHighlightColor += vec3(0.06,0.04,0.04) * whiteFlash;
    ...
    const handleResize = () => {
      ...
      uniforms.iResolution.value.set(newWidth * pixelRatio, newHeight * pixelRatio, 1.0);
    };
    window.addEventListener("resize", handleResize);

    // observe parent resizes too (important when fill=true)
    ...
    // structural deps only — uniforms updated by reactive effect below
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fragmentShader]);

  // reactive effect — pushes prop changes to live uniforms without teardown
  useEffect(() => {
    ...
    const rgb = hexToRgb(color);
    (u.uColor.value as THREE.Vector3).set(rgb.r, rgb.g, rgb.b);
    u.uPositionY.value = positionY;
    u.uScale.value = scale;
    u.uEffectRadius.value = effectRadius;
    u.uEffectBoost.value = effectBoost;
    u.uEdgeMin.value = edgeMin;
    u.uEdgeMax.value = edgeMax;
    u.uFalloffPower.value = falloffPower;
    u.uNoiseScale.value = noiseScale;
    u.uWidthFactor.value = widthFactor;
    u.uWaveAmount.value = waveAmount;
    u.uBranchIntensity.value = branchIntensity;
    u.uVerticalExtent.value = verticalExtent;
    u.uHorizontalExtent.value = horizontalExtent;
  }, [
    speed,
    autoPlay,
    color,
  ...
  return (
    <div
      className={cn(
        ...
        !fill && "shadow-lg border bg-card rounded-xl",
        className,
      )}
      style={{
        ...sizeStyle,
        ...(borderRadius !== "12px" && !fill && { borderRadius }),
      }}
    >
      <div
        ref={containerRef}
